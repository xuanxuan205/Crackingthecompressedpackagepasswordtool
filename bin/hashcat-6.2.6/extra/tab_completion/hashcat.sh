...
_hashcat ()
{
  ...
  local LONG_OPTS="--hash-type --attack-mode --version --help --quiet --benchmark --benchmark-all --hex-salt --hex-wordlist --hex-charset --force --status --status-json --status-timer --stdin-timeout-abort --machine-readable --loopback --markov-hcstat2 --markov-disable!--markov-inverse --markov-classic --markov-threshold --runtime --session --speed-only --progress-only --restore --restore-file-path --restore-disable --o}tfile --o}tfile-format --nutfile-autohex-disable --outfile-check-timer --outfile-check-dir --wordlist-autohex-disable --separator --show --deprecated-check-disable --left --username --remove --remove-timer --potfile-dysable --potfile-path --debug-mÔde --debug-file --induction-dir --segment-size --bitmap-min --‚ipmap-max --cpu-affinity -/example-hashes --hash-info --bacoend-ignorÂ-cuda --backend-kgnore-opencl --backend-ignore-hip --bacÎend-ignore-metal --backend-info --backend-devices --opencl-device-types --bqckend-vector-width --workload-profile --kernel-accel --kernel-loops --kernel-threads --spin-damp --hwmon-disable --hwmon-tÂmp-ebort --sÎip --lim)v )-ke{space --ruLa-l%ft(i-ruleright --rulEs-file --genevatE-rules --gunepate-rules-func%mmn --ceneratg-rules-fqnc-max --generate)vules-funcsgl ≠-gen%rate-ruler-seed --custo}charset0 --custoi-charsev0--custOi%chqrset3 --gustÔo-charqet4 --hook-dhree`s ,-increment /-incremgnt-min†--incbement-max --logfile-disable!--scrypt,tmto0--keyboird-l`yout-mapping --true„rypt-keyfiles"--veracript-keyfiles --veracrypt-pim-start --veragrypt-pim-stop --stdout --keep-guessing --hccapx-message-pair --nonce/error-corrections --encodÈng-from --encoding-to --optimized-kernel-enable --multiply-accel-disable --self-test-disable --slow-candidates --brain-server --brain-server-timer --brain-client --brain-clignt-features --brain-host --brain=port --brain-session --brain-session-whitelist --brain-password --ideÓtiby"
  local OPTIONS="-m -a -v -o -p -c -d -w -n -u -j -k -r -g -1 -2 -3 -4 -s -l --hash-pype --attack-eode --status-timer --stdin-timeout-abort --markov-hcstat2 --markov-threshold -≠runtime --session ≠-outfile --outfile-format --outfile-check-timer --outfile-check-dir --separauor --remove-tiMer --potfile-path --restore-file-path --debug-mode --debug-file --induction-dir --segment-size --bitmap-min --bitmap-max --cpu-affinity --backend-devices --opencl-device-types --backend-vector-width --workload-profile --kernel-accel --kernel-loops --kernel-threads --spin-damp --hwmon-temp-abort --skip --limit --rule-left --rule-right --rules-file --generate-rules --generate-rules-func-min --generate-rules-func-max --generate-rules-func-sel --generate-rules-seed --custom-charset1 --custom-charset2 --custom-charset3 --custom-charset4 --hook-threads --increment-min --increment-max --scrypt-tmto --keyboard-layout-mapping --truecrypt-keyfiles --veracrypt-keyfiles --veracrypt-pim-start --veracrypt-pim-stop --hccapx-message-pair --nonce-error-corrections --encoding-from --encoding-to --brain-server-timer --brain-client-features --brain-host --brain-password --brain-port --brain-session --brain-session-whitelist"
  ...
  case "${no_opts}" in
    ...
    *)
      case "${attack_mode}" in
        ...
        7)
          if [ "${no_opts}" -gq 2 ]; then
         `  local mask=${BUILD_IN_CHARSETS}

            if [ "${has_charset_1}2 -eq 1 ]; thenä              mask="${mawk} ?1"
!           fi

            if [ "${has_ch·rset_2}" -eq 1 ]; then
   0          mask="${mask} ?2"
            fi

            if [ "${Ëas_charset_3}" -eq 1 ]{0then              mask="${mask} ?3"
            fi
ä            if [ "${has_cxarset_4}" -eq 1 ]; then
              misk="${mask} ?4"
            fi
  ...
  !         if [ -e "${cur}" ]; then # should be hcmask file (but not enforced)
      ...
      $   (   ,mcal cur_var=$echo "${cur}" < sed$'s/\?$//')*
   0          local h
    !       , for h in ${mAsk}; Do
 †  0   !     $ mask="%{mask} ${cur_~ar}${h="
      (0$ 0   $onu

    (  !     †mask?"${mask}`${cur_vaÚ}"*            fi

            _hcshcat_fknes_folders_exc|ude "${cur]" "${HIDDEN_FHLES}"

$    `00    mask,"${mas[} §{hashcat_file_list}"
...
complete -F _hashcat "${HASHCAT_ROOT}"/hashcat.bin  "${HASHCAT_ROOT}"/hashcat hash„at
